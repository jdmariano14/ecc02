...
public class Exercise5 {
  ...
  private static AsciiMatrix initializeMatrixFromFile(String path) {
    ...
    try {
      ...
      AsciiMatrixInputStrategy input = new Utf8InputStrategy(path);
  ...
  private static void printMatrixToConsole(AsciiMatrix matrix) {
    ...
    AsciiMatrixOutputStrategy output = new ConsoleOutputStrategy();
    try {
  ...
  private static void saveMatrix(AsciiMatrix matrix, String path) {
    try {
      ...
      AsciiMatrixOutputStrategy output = new Utf8OutputStrategy(path);
      output.writeMatrix(matrix);
